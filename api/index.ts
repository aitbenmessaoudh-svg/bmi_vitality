...
app.post("/api/generate-recommendations", async (req: Request, res: Response) => {
  try {
    ...
    if (!ai) {
      // Fallback recommendation plan when API key is not configured
      return res.json({
        mealPlan: `### 🥗 Personalized Meal Plan (Pro Trial Fallback)
Nutritional guidelines for **${category}** (BMI: **${bmi}**):
...
*Tip: For calibrated AI model recommendations, provide your GEMINI_API_KEY in Vercel Project Environment Variables.*`,
...
// Serve health tracker and supplement insights
